...
## ✅ Inconsistencies Found & Fixed — Web Verification (March 2026)

Cards where seed data differed from verified web sources (Doctor of Credit, US Credit Card Guide, US Card Forum).
All items below have been applied to `SeedData.java`. SEED_VERSION bumped to 14.

---

### 1. Delta SkyMiles Reserve Amex (`delta_reserve_amex`)

| Field | Code Has | Web Data |
|---|---|---|
| Dining rate | 2x Miles | **1x Miles** |

**Notes**: Delta Reserve earns 3x on Delta purchases only, 1x on all other spend — no Dining bonus.
**Fix applied**: Removed 2x DINING row from SeedData.java.

---

### 2. Delta SkyMiles Platinum Amex (`delta_platinum_amex`)

| Field | Code Has | Web Data |
|---|---|---|
| Groceries | *(missing)* | **2x Miles** |

**Notes**: Delta Platinum earns 3x Delta, 2x Dining, 2x US supermarkets, 1x everything else.
**Fix applied**: Added 2x GROCERIES row to SeedData.java.

---

### 3. Hilton Honors Amex Aspire (`hilton_aspire_amex`)

| Field | Code Has | Web Data |
|---|---|---|
| Travel (flights / car rentals) | *(missing)* | **7x Points** |

**Notes**: Aspire earns 14x Hilton hotels, 7x Dining, 7x Flights & car rentals booked direct, 3x General.
**Fix applied**: Added 7x TRAVEL row to SeedData.java.

---

### 4. Marriott Bonvoy Brilliant Amex (`marriott_brilliant_amex`)

| Field | Code Has | Web Data |
|---|---|---|
| Travel (airlines / taxis) | *(missing)* | **3x Points** |
| Groceries | *(missing)* | **3x Points** |

**Notes**: Brilliant earns 6x Marriott, 3x Dining + Airlines + Taxis + US supermarkets, 2x everything else.
**Fix applied**: Added 3x TRAVEL and 3x GROCERIES rows to SeedData.java.

---

### 5. United Quest Card (`united_quest`)

| Field | Code Has | Web Data |
|---|---|---|
| Travel (general — non-United hotels/airlines) | *(missing)* | **2x Miles** |

**Notes**: Quest earns 3x United, 2x General Travel (non-United), 2x Dining, 1x General.
**Fix applied**: Added 2x TRAVEL row to SeedData.java.

---

### 6. United Club Card (`united_club`)

| Field | Code Has | Web Data |
|---|---|---|
| Travel (general — non-United hotels/airlines) | *(missing)* | **2x Miles** |

**Notes**: Club earns 4x United, 2x General Travel, 2x Dining, 1x General.
**Fix applied**: Added 2x TRAVEL row to SeedData.java.

---

### 7. World of Hyatt Credit Card (`world_of_hyatt`)

| Field | Code Has | Web Data |
|---|---|---|
| Travel (airlines / transit / gyms) | *(missing)* | **2x Points** |

**Notes**: Hyatt earns 4x Hyatt stays, 2x Airlines + Transit + Fitness clubs, 2x Dining, 1x General.
**Fix applied**: Added 2x TRAVEL row to SeedData.java.

---

### 8. IHG One Rewards Premier (`ihg_premier`)

| Field | Code Has | Web Data |
|---|---|---|
| Gas | 3x Miles | **5x Miles** |
| Groceries | *(missing)* | **5x Miles** |
| General (base rate) | 2x Miles | **3x Miles** |

**Notes**: IHG Premier earns 10x IHG, 5x Dining, 5x Gas, 5x Grocery, 3x everywhere else.
**Fix applied**: Gas changed to 5x, added 5x GROCERIES, GENERAL base changed to 3x in SeedData.java.

---

### 9. Citi Strata Elite (`citi_strata_elite`)

| Field | Code Has | Web Data |
|---|---|---|
| Travel | 3x Points | **3x Hotels only** — needs verification |

**Notes**: Strata Elite earns 12x portal, 3x Hotels, 3x Dining, 1.5x General. Hotels fall under our TRAVEL category — the 3x TRAVEL row is correct for hotel-related spend. No code change needed.

---

### 10. US Bank Altitude Reserve (`usbank_altitude_reserve`)

| Field | Code Has | Web Data |
|---|---|---|
| All rates & credits | 5% Travel, 3% Dining, $325 Credit | **Needs re-verification after Dec 2025 restructure** |

**Notes**: As of Dec 15, 2025: 5x on prepaid hotels/car rentals via Altitude Rewards Center portal, 3x on travel (all other travel) + mobile wallet, 1x general. Points now worth 1¢ each (down from 1.5¢). $325 credit covers travel + dining. Card is no longer open to new applicants.
**Fix applied**: TRAVEL 5%→3%, DINING bonus removed, TRAVEL_PORTAL 5% added to SeedData.java.

---

### 11. Southwest Rapid Rewards Priority (`southwest_priority`)

| Field | Code Has | Web Data |
|---|---|---|
| Southwest Travel Credit | $75 Annual | **EXPIRED Dec 31, 2025** |
| Upgraded Boardings | *(in seed)* | **EXPIRED Dec 31, 2025** |

**Notes**: SW overhauled all Rapid Rewards cards Jan 1, 2026. Previous Priority benefits ($75 travel credit, 4 upgraded boardings) were replaced with 7,500 annual anniversary bonus points.
**Fix applied**: Removed expired benefits, added 7,500 Anniversary Bonus Points to SeedData.java.

---

### 12. Southwest Rapid Rewards Plus (`southwest_plus`)

**Notes**: Same SW Jan 2026 overhaul — EarlyBird Check-In replaced by Southwest's new assigned seating system.
**Fix applied**: Removed EarlyBird Check-In from SW Plus (and SW Premier) in SeedData.java.

---

### 13. Chase Ritz-Carlton (`chase_ritz_carlton`) — *New card, added March 2026*

| Field | Code Has | Web Data |
|---|---|---|
| Hotel Credit | *(missing)* | **$100 Annual** (2+ consecutive nights at Ritz-Carlton) |

**Notes**: Ritz-Carlton has a $100 on-property hotel credit for 2+ night stays, but this is an FHR-style benefit (not a statement credit) — not tracked in the app. No code change needed.

---

### 14. Chase Sapphire Preferred (`chase_sapphire_preferred`)

| Field | Code Has | Web Data |
|---|---|---|
| Peloton Credit | *(missing)* | **$60 Annual** (possibly — verify current status) |

**Notes**: User confirmed the Peloton credit is no longer offered on CSP. No credit was in the seed to begin with — nothing to change.

---

*Cards that were updated in the seed during March 2026 but whose entries in this file were outdated — corrected below:*
- **Amex Business Platinum**: fee updated $695 → $895 in seed *(corrected in entry below)*
- **Capital One Savor**: changed to Capital One Miles + added 5x Travel Portal *(corrected in entry below)*
- **US Bank Altitude Connect**: added 5x Travel Portal, 2x Entertainment, $100 GE/TSA benefit *(corrected in entry below)*
- **New cards added**: Chase Ritz-Carlton, Capital One VentureOne, Capital One QuicksilverOne, US Bank Smartly, HSBC Premier, HSBC Elite *(entries added below)*
